...
$selecionarlistamaquinado="SELECT * FROM MAQUINADOPIEZA WHERE MAQUINADO_idMAQUINADO=$idmaquinado";
$querylista=mysqli_query($conn,$selecionarlistamaquinado);

echo '<div class="row">';
      echo '<div class="col-md-12">';
      //-------------------------------------seleciuonamos los tiempos--------------
     $selecionartareas="SELECT * FROM TIEMPOSMAQUINADO WHERE MAQUINADO_idMAQUINADO=$idmaquinado AND Final IS NULL";
     ...
     ///------------------------fin los tiempos
      ...
      while ($filalista=mysqli_fetch_array($querylista))
      {
        ...
        echo '<td>'.$filalista['Nombre'].'</td>';
        echo '<td>'.$filalista['Medidas'].'</td>';
        echo '<td>'.$filalista['Material'].'</td>';
        echo '</tr>';
      }
      echo '</tbody>';
      echo '</table>';
